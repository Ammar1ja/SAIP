...
import IpEnforcementContentPage from './IpEnforcementContent';
import { getIPObservatoryEnforcementPageData } from '@/lib/drupal/services';
import FeedbackSection from '@/components/organisms/FeedbackSection';

export default async function IpEnforcementPage({ params }: PageProps) {
  const { locale } = await params;
  const messages = await getMessages({ locale });
  const data = await getIPObservatoryEnforcementPageData(locale);
  ...
  const breadcrumbItems = [
    ...
    {
      label: messages.breadcrumbs?.ipEnforcement || 'IP enforcement',
    },
  ];
  ...
  return (
    <main>
      <HeroStatic
        ...
        textColor="dark"
      />
      <IpEnforcementContentPage data={data} />
      <FeedbackSection />
    </main>
  );
}
